...
int main(void)
{
    ...
    unsigned int myKeyAgreementKeyID;

    unsigned int *mySignKeyID = (unsigned int *)malloc(sizeof(unsigned int));
    if (NULL == mySignKeyID)
    {
        printf("Failed to malloc a Sign Key ID\n");
        goto exit;
    }
    *mySignKeyID = 4;

    // 32-bytes secret
    uint8_t *secret = (uint8_t *)"12345678901234567890123456789011";

    printf("My sign key ID : %d\n", *mySignKeyID);
    ...
    JWK *mySignJWK2 = iotex_jwk_generate(JWKTYPE_EC, JWK_SUPPORT_KEY_ALG_P256,
    ...
    char *mySignDID2 = iotex_did_generate("io", mySignJWK2);
    if (mySignDID2)
        printf("My Sign DID : \t\t\t%s\n", mySignDID2);
    else
        goto exit;
    ...
    psa_status_t psa_status = psa_destroy_key(*mySignKeyID);

    if (PSA_SUCCESS != psa_status)
        printf("Failed to destroy the key\n");
